...
import { ArrowLeft, Download, FileSpreadsheet, BarChart3, Users, CheckCircle2, XCircle, MinusCircle, RefreshCw, Search, Filter } from 'lucide-react';
  ...
  const fetchResultData = async () => {
    ...
    try {
      ...
      const analysis = realQuestions.map((q, index) => {
        ...
        let wrong = 0;
        let blank = 0;
        // Objek penampung pilihan jawaban (A-E)
        ...
        finishedSessionIds.forEach(sId => {
          ...
          if (!studentAns || !studentAns.chosen_answer) {
            ...
            if (distro.hasOwnProperty(chosen)) distro[chosen]++; // Hitung sebaran
        ...
        {activeTab === 'rekap' ? (
                  ...
                  {filteredParticipants.map((p, idx) => {
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors">
                        ...
                        <td className="p-5">
                          <span className={`text-[10px] font-black px-3 py-1 rounded-lg uppercase ${session?.status === 'finished' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
                            {session ? (session.status === 'finished' ? 'Selesai' : session.status === 'locked' ? 'Terkunci' : 'Aktif') : 'Belum Mulai'}
                          </span>
                        </td>
                        <td className="p-5 text-right font-black text-xl">{session?.status === 'finished' ? session.score : '-'}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-[2rem] border border-slate-100 dark:border-zinc-800 shadow-sm overflow-hidden">
             <div className="p-6 border-b border-slate-100 dark:border-zinc-800">
                ...
                <p className="text-[10px] text-slate-400 font-bold mt-1">Siswa Selesai: {participants.map(p => getBestSessionForExport(p.id)).filter(s => s?.status === 'finished').length}</p>
             </div>
             <div className="overflow-x-auto">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-zinc-800">
                  {analysisData.map((item) => (
                    <tr key={item.no} className="hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors">
                      ...
                      <td className="p-5">
                        <p className="text-xs font-bold text-slate-600 dark:text-slate-400 line-clamp-1 mb-3">{item.question_text}</p>
                        <div className="flex gap-1.5">
                          {['A', 'B', 'C', 'D', 'E'].map(opt => (
                            <div key={opt} className="flex flex-col items-center bg-slate-50 dark:bg-zinc-800 px-2 py-1 rounded-lg border border-slate-100 dark:border-zinc-700 min-w-[32px]">
                              <span className="text-[8px] font-black text-slate-400">{opt}</span>
                              <span className={`text-[10px] font-black ${item.distro[opt] > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-300 dark:text-zinc-600'}`}>{item.distro[opt]}</span>
                            </div>
                          ))}
                        </div>
                      </td>
                      <td className="p-5 text-center font-black text-emerald-600">{item.correct}</td>
                      <td className="p-5 text-center font-black text-red-500">{item.wrong}</td>
                      <td className="p-5 text-center font-black text-slate-400">{item.blank}</td>
                      <td className="p-5 text-right pr-8">
                        <span className={`text-sm font-black ${item.correctPercentage >= 75 ? 'text-emerald-500' : 'text-orange-500'}`}>{item.correctPercentage}%</span>
                      </td>
                    </tr>
                  ))}
                </tbody>

...
import { Lock, LogIn, Sun, Moon, GraduationCap, User as UserIcon, Eye, EyeOff, Sparkles, UserCog, BookOpen, Users, School } from 'lucide-react';
    ...
    try {
      ...
      if (role === 'siswa') {
        ...
        if (student) {
          userData = {
            ...
            fullName: student.full_name || 'Demo Siswa',
            nis: student.nis,
            role: 'siswa'
          };
          redirectPath = '/student-dashboard';
        } else {
          throw new Error("Tidak ada data siswa demo yang tersedia!");
        }
      } else {
        // Login as demo teacher/admin/kurikulum
        ...
        if (staff) {
          userData = {
            ...
            fullName: staff.full_name || `Demo ${role.charAt(0).toUpperCase() + role.slice(1)}`,
            role: staff.role_level ? staff.role_level.toLowerCase() : role,
            email: staff.email
          };
          redirectPath = '/dashboard';
        } else {
          throw new Error(`Tidak ada data ${role} demo yang tersedia!`);
        }
      }
      ...
      if (userData) {
        ...
        Swal.fire({
          ...
          text: `Selamat datang sebagai ${role.toUpperCase()}, ${userData.fullName}`,
          icon: 'success',
          timer: 1500,
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 flex flex-col justify-center items-center p-6 transition-colors duration-500 font-sans text-left relative overflow-hidden">
      ...
      <div className="w-full max-w-2xl bg-white/90 dark:bg-zinc-900/90 backdrop-blur-xl p-10 rounded-[3rem] shadow-2xl shadow-slate-200/50 dark:shadow-black/50 border border-white/20 dark:border-zinc-800 transition-colors duration-500 relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        <div className="text-center mb-10">
          ...
          <div className="mt-4 inline-block px-4 py-2 bg-orange-600/10 border border-orange-600/20 rounded-full">
            <p className="text-xs text-orange-600 dark:text-orange-400 font-bold uppercase tracking-wider">🎯 Demo Mode</p>
          </div>
        </div>
          ...
          <>
            ...
            <div className="text-center mt-6">
              <button
                ...
                className="text-sm text-slate-500 dark:text-zinc-400 hover:text-orange-600 dark:hover:text-orange-400 font-bold uppercase tracking-wider transition-colors underline"
              >
                atau login manual
              </button>
            </div>
